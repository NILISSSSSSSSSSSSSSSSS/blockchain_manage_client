<template>
    <div class="container">
        <bread title="接口转账记录" desc="管理接口的转账记录" :list="breadNav"></bread>

        <div class="record-box">
            <div class="record">
                <header class="head">接口转账记录列表</header>
                <div class="search">
                ...
                <div class="table">
                    <el-table ref="table" :data="tableData" :header-cell-style="{color: 'rgba(0,0,0,.85)','text-align': 'center'}" :cell-style="{'text-align': 'center'}" style="width: 100%">
                        ...
                        <el-table-column prop="remark" label="备注">
                            <!-- <template slot-scope="scope">
                                <el-popover trigger="hover" placement="top" v-show="scope.row.remark">
...
import { getTransferApiRecord } from "api/api";
...
export default {
    ...
    data() {
        return {
            breadNav: [
                { name: "站内交易", path: "" },
                { name: "接口转账记录", path: "/deal/transfer/record" }
            ],
            search: {
                currency: "",
                startDate: "",
    ...
    methods: {
        ...
        _getTransferRecord() {
            ...
            getTransferApiRecord(searchObj).then(res => {
                if (res.code == 0) {
